...
* **CURRENT STATE 2026-06-23 (session 13) -- PREWARM = STRONG (UNCONFIRMED) STALL-CURE LEAD + the full Phase A symmetric-kernel arc. Board: build 2896 at 192.168.0.8 (prewarm ON, healthy). Detail: [[project_stall_session12]] + docs/NEXT_20260623_symmetric_kernel_redesign.md; seed prompt in that doc S11.** Sessions 11-12 (see memory) refuted the blocking-sleep cure and PIVOTED to a SYMMETRIC/core-agnostic kernel (survive a per-core wedge, not prevent it). This session built that out AND, via its confinement gate, found a likely CURE:
  - **Phase A shipped (commits on main; Bryan pushes): allocator LOCK + UN-PIN servers behind /proc/distribute + /proc/vkalock (293d006); surgical distribution + /proc/pincore + per-server core listing (3cb91f4); a load-aware PLUGGABLE placement policy /proc/placement (strategies extracted to include/aios/sched_strategy.h, host-tested 16/16, 2a0a4f7); the HW confinement-gate worker /proc/confine (57c88d6).** Board revived this session (dead 2881 -> built + balenaEtchered a clean SD image -> build 2885 -> flash-free pi_flash to the gate kernels).
  - **HW CONFINEMENT GATE RESULT (decisive, SPLIT): the ~32.4s wedge splits by type. The IDLE->WAKE wedge ([STAGECP] core=0 prev=9 this=11 = a cold load INSIDE the seL4 handler, AFTER NODE_LOCK) holds the BKL -> CLUSTER-FREEZE (a syscall-doing worker on core 2 froze, advanced=0, co-wedged iovf=1). The TEARDOWN-reap wedge (prev=13 this=9 = post-eret user fetch, pre-lock) is CONFINED (the worker ran 683k syscalls through a 32s wedge). Reconciles the s8 (BKL-held) vs s12 (BKL-free) contradiction -- both right for different wedge types.**
  - **THE PREWARM ATTACK -> HW: 0 WEDGES (commit 0a4d045). FIX: relocate the first cold fabric op to BEFORE NODE_LOCK at each seL4 kernel entry (aios_fabric_prewarm() = a per-core cold-scratch read + dsb sy; deps/kernel c_traps.c/errata.c/traps.h; the 4 slowpath/IRQ entries, NOT the IPC fastpath; default-ON AIOS_FABRIC_PREWARM). A/B (same board/cycles, only the prewarm differs, serial-independent /proc/laststall): prewarm kernel build 2896 = ZERO core-0 wedges over ~14 idle->teardown cycles vs ~6 on the no-prewarm build 2893. Worker-confound ruled out (armed in both); teardowns confirmed firing (ASIDGEN climbed). => the prewarm appears to PREVENT the wedge, not just confine it -- coherent because it is CORE 0's OWN per-entry ACE/snoop-master touch, the one thing every prior keep-warm (DMA/coherent/fabwarm/GENET, all secondary/external) could NOT do (s8: the quiesce is the A72 cluster's OWN master interface). DISTINCT from the REFUTED s10 IWARM (that PREVENTED via L2 line-holding; this RELOCATES the one-time fabric wake).**
  - **NOT A DECLARED CURE (12 sessions refuted cures -- demand extraordinary proof). CONFIRM NEXT (the seed): (1) REPRODUCIBILITY re-flash A/B (flash no-prewarm -> wedges RETURN via /proc/laststall -> flash prewarm -> gone); (2) longer-idle + more-cycle soak; (3) a RUNTIME prewarm on/off toggle for a same-boot A/B (needs a kernel<->userspace channel -- the flag is a kernel global; non-trivial); (4) measure the dsb-sy-per-slowpath-entry PERF cost + try a lighter barrier. METHOD: the FTDI adapter is BUMP-SENSITIVE (dropped twice mid-run) -> use /proc/laststall (netconsole) as the serial-independent wedge counter; /proc/confine.2 arms the probe worker, .r disarms; pi_flash.py --build for flash-free kernel swaps; FULL QEMU gate before every flash (SMP 4/5 baseline). STILL A MAJOR OPEN CONCERN ([[feedback_stall_open_concern]]) -- even if confirmed, validate broadly before concluding.**

* **CURRENT STATE 2026-06-22 (session 10) [SUPERSEDED by session 13 above] -- RE-LOCALIZED REGISTER-EXACT ON HW + DECISIVELY ANSWERED:
