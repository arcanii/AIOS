#!/bin/sh
# gate.sh -- the AIOS userspace-kernel acceptance gate, NATIVE (no docker).
#
# Builds the whole suite, then runs the 16-key gate TWICE -- once per PAL backend (PAL=linux then
# PAL=seccomp) -- proving the SAME host-agnostic kernel passes over two host trap mechanisms. Runs
...
make --no-print-directory clean && make --no-print-directory -j"$J" all || exit 1
gate() {
           ...
           echo "=== clean shutdown -- reboot() is ROOT-only; poweroff/halt/reboot exit aios-uk with a power code ===" &&
           ./aios-uk ./prog_reboot </dev/null 2>/dev/null | sed "s/^/    /";
           ./aios-uk ./prog_reboot </dev/null >/dev/null 2>&1; rbrc=$?; echo "  [prog_reboot exit $rbrc (expect 0 -- unprivileged reboot denied)]";
           ./aios-uk ./poweroff >/dev/null 2>&1; poff=$?; echo "  [poweroff (root) -> aios-uk exit $poff (expect 200 = AIOS_EXIT_POWEROFF)]";
           cp poweroff reboot 2>/dev/null; ./aios-uk ./reboot >/dev/null 2>&1; rbt=$?; rm -f reboot; echo "  [reboot (root)  -> aios-uk exit $rbt (expect 202 = AIOS_EXIT_REBOOT)]";
           test "$rbrc" = 0 && test "$poff" = 200 && test "$rbt" = 202; shrc=$?;
           ...
           echo "=== float printf (%f/%e/%g) -- libaios printf matches glibc byte-for-byte + seq/printf UNMODIFIED ===" &&
           cc -O2 -w -o /tmp/host_printf guest/prog_printf.c 2>/dev/null;
           ./aios-uk ./prog_printf >/tmp/aios_printf.out 2>/dev/null; /tmp/host_printf >/tmp/host_printf.out 2>/dev/null;
           if diff -q /tmp/host_printf.out /tmp/aios_printf.out >/dev/null 2>&1; then pfrc=0; echo "  [prog_printf == glibc byte-for-byte: PASS]"; else pfrc=1; echo "  [prog_printf DIFFERS from glibc: FAIL]"; diff /tmp/host_printf.out /tmp/aios_printf.out | head; fi;
           printf "  seq 0 0.5 2: "; ./aios-uk ./sbase-seq 0 0.5 2 2>/dev/null | tr "\n" " "; echo;
           printf "  printf %%.2f %%s: "; ./aios-uk ./sbase-printf "%.2f=%s\n" 3.14159 pi 2>/dev/null;
           ...
           echo "--- interactive dash + ^C over a pty (the kernel routes SIGINT to the foreground group) ---" &&
           cc -O2 -o /tmp/ctrlc_pty test/ctrlc_pty.c -lutil 2>/dev/null &&
           timeout 25 /tmp/ctrlc_pty ./aios-uk ./dash 2>/dev/null; cprc=$?;
           ...
           echo "--- RAW terminal mode -- tcsetattr: read one keypress, no Enter, no echo ---" &&
           cc -O2 -o /tmp/rawkey_pty test/rawkey_pty.c -lutil 2>/dev/null &&
           timeout 20 /tmp/rawkey_pty ./aios-uk ./prog_rawkey 2>/dev/null; rkrc=$?;
           ...
           echo "=== networking (timed read) -- SO_RCVTIMEO: a parked socket read expires -> EAGAIN (DNS timeout groundwork) ===" &&
           timeout 20 ./aios-uk ./prog_rcvtimeo 2>/dev/null | grep -vE "^\[aios-uk\]" | sed "s/^/  /";
           timeout 20 ./aios-uk ./prog_rcvtimeo >/dev/null 2>&1; rtrc=$?; echo "  [prog_rcvtimeo exit $rtrc (expect 0)]";
           echo "=== networking (DNS) -- a from-scratch AIOS resolver (gethostbyname/getaddrinfo over UDP) resolves a name via a host DNS stub ===" &&
           cc -O2 -o /tmp/dns_server test/dns_server.c 2>/dev/null &&
           timeout 20 /tmp/dns_server ./aios-uk ./prog_dns 2>/dev/null | grep -vE "^\[aios-uk\]" | sed "s/^/  /";
           cc -O2 -o /tmp/dns_server test/dns_server.c 2>/dev/null; timeout 20 /tmp/dns_server ./aios-uk ./prog_dns >/dev/null 2>&1; dnsrc=$?; echo "  [dns_server exit $dnsrc (expect 0)]";
           echo "=== networking (confinement) -- AIOS_NET_ALLOW gates outbound connect: in-list allowed, out-of-list + MALFORMED rules refused (EACCES, fail-closed) ===" &&
           cc -O2 -o /tmp/net_jail test/net_jail.c 2>/dev/null &&
           timeout 30 /tmp/net_jail ./aios-uk ./prog_netjail 2>/dev/null | grep -vE "^\[aios-uk\]" | sed "s/^/  /";
           cc -O2 -o /tmp/net_jail test/net_jail.c 2>/dev/null; timeout 30 /tmp/net_jail ./aios-uk ./prog_netjail >/dev/null 2>&1; njrc=$?; echo "  [net_jail exit $njrc (expect 0)]";
           echo "=== networking (bind/listen confinement) -- AIOS_NET_BIND_ALLOW gates which local addr/port a guest may bind+listen; unbound listen auto-bind refused; malformed rules FAIL CLOSED ===" &&
           cc -O2 -o /tmp/bind_jail test/bind_jail.c 2>/dev/null &&
           timeout 30 /tmp/bind_jail ./aios-uk ./prog_bindjail 2>/dev/null | grep -vE "^\[aios-uk\]" | sed "s/^/  /";
           cc -O2 -o /tmp/bind_jail test/bind_jail.c 2>/dev/null; timeout 30 /tmp/bind_jail ./aios-uk ./prog_bindjail >/dev/null 2>&1; bjrc=$?; echo "  [bind_jail exit $bjrc (expect 0)]";
           echo "=== gate: pipebig jail execjail clock pcwd umask regex pwgrp id crypt printf shutdown net netsrv netloop rcvtimeo dns netjail bindjail jobctl stop sigmask sigpipe ctrlc ctrlc-job ctrlz rawkey login must exit 0 ===" &&
           ./aios-uk ./prog_pipebig >/dev/null 2>&1; rc=$?; echo "  [pipebig $rc, jail $jrc, execjail $ejrc, clock $clkrc, pcwd $pcwdrc, umask $umrc, regex $rxrc, pwgrp $pwrc, id $idrc, crypt $cryrc, printf $pfrc, shutdown $shrc, net $netrc, netsrv $nsrc, netloop $nlrc, rcvtimeo $rtrc, dns $dnsrc, netjail $njrc, bindjail $bjrc, jobctl $jcrc, stop $strc, sigmask $smrc, sigpipe $sprc, ctrlc $cprc, ctrlc-job $cjrc, ctrlz $czrc, rawkey $rkrc, login $lprc]";
           test "$rc" = 0 && test "$jrc" = 0 && test "$ejrc" = 0 && test "$clkrc" = 0 && test "$pcwdrc" = 0 && test "$umrc" = 0 && test "$rxrc" = 0 && test "$pwrc" = 0 && test "$idrc" = 0 && test "$cryrc" = 0 && test "$pfrc" = 0 && test "$shrc" = 0 && test "$netrc" = 0 && test "$nsrc" = 0 && test "$nlrc" = 0 && test "$rtrc" = 0 && test "$dnsrc" = 0 && test "$njrc" = 0 && test "$bjrc" = 0 && test "$jcrc" = 0 && test "$strc" = 0 && test "$smrc" = 0 && test "$sprc" = 0 && test "$cprc" = 0 && test "$cjrc" = 0 && test "$czrc" = 0 && test "$rkrc" = 0 && test "$lprc" = 0;
}
